...
4. **Thanh toán và Đặt hàng (Checkout):**
   ...
   - Hệ thống cho phép nhập mã Khuyến mãi (Voucher) hợp lệ. Nếu đủ điều kiện (ví dụ: Tổng đơn hàng lớn hơn mức quy định), hệ thống tự động trừ tiền chiết khấu. Đặc biệt, khách hàng có thể áp dụng ĐỒNG THỜI cả 1 mã Miễn phí vận chuyển (Freeship) và 1 mã Giảm giá (Discount) trên cùng một đơn hàng.
...
Hệ thống sử dụng cơ sở dữ liệu quan hệ (RDBMS) chuẩn mực với các ràng buộc khóa ngoại (FOREIGN KEY) chặt chẽ, tự động xử lý mồ côi dữ liệu thông qua ON DELETE CASCADE và ON DELETE SET NULL. Hiện tại cơ sở dữ liệu (version 13) bao gồm 9 bảng chính được liên kết với nhau:
...
2. **Bảng `categories` (Danh mục sản phẩm):**
   - `id` (INTEGER PRIMARY KEY AUTOINCREMENT)
   - `name` (TEXT), `imageResId` (INTEGER - lưu dạng ID hoặc đường dẫn)

3. **Bảng `products` (Sản phẩm):**
   - `id` (INTEGER PRIMARY KEY AUTOINCREMENT)
   - `name`, `categoryName`, `color`, `sizes`, `description` (TEXT)
   - `categoryId` (INTEGER): Khóa ngoại liên kết tới bảng categories.
   - `price`, `stock`, `imageResId` (INTEGER)

4. **Bảng `product_images` (Hình ảnh phụ của sản phẩm):**
   - `id` (INTEGER PRIMARY KEY AUTOINCREMENT)
   - `productId` (INTEGER): Khóa ngoại tới products.
   - `imagePath` (TEXT), `sortOrder` (INTEGER)

5. **Bảng `vouchers` (Mã khuyến mãi):**
   - `id` (INTEGER PRIMARY KEY AUTOINCREMENT)
   - `code` (TEXT UNIQUE)
   - `type` (TEXT), `value`, `minOrder`, `usageLimit`, `usedCount` (INTEGER)

6. **Bảng `cart_items` (Giỏ hàng tạm thời):**
   - `id` (INTEGER PRIMARY KEY AUTOINCREMENT)
   - `userId` (INTEGER), `productId` (INTEGER)
   - `quantity` (INTEGER), `selectedColor`, `selectedSize` (TEXT)

7. **Bảng `orders` (Hóa đơn / Đơn hàng):**
   - `id` (INTEGER PRIMARY KEY AUTOINCREMENT)
   - `code` (TEXT)
   - `userId` (INTEGER): Khóa ngoại người đặt.
   - `receiverName`, `phone`, `address`, `note`, `createdAt`, `status`, `freeshipVoucherCode`, `discountVoucherCode` (TEXT)
   - `total` (INTEGER)

8. **Bảng `order_details` (Chi tiết hóa đơn):**
   - `id` (INTEGER PRIMARY KEY AUTOINCREMENT)
   - `orderId` (INTEGER), `productId` (INTEGER)
   - `productName`, `selectedColor`, `selectedSize` (TEXT)
   - `quantity`, `unitPrice`, `subtotal` (INTEGER): Lưu giá tại thời điểm mua.

9. **Bảng `reviews` (Đánh giá):**
   - `id` (INTEGER PRIMARY KEY AUTOINCREMENT)
   - `userId`, `productId` (INTEGER)
   - `rating` (INTEGER), `comment`, `createdAt` (TEXT).
